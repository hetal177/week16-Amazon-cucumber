...
public class Utility extends ManageDriver {
    ...
    // NOTE: Should you make new method for utility please follow below steps:
    // 1) name the method starting with 'do'
    // 2) test it - if it works then share it with team so they can add it in their utility
...
//******************************JAVA Executor Methods*********************************
     ...
     */
    public void JavaScriptExecutorExecuteASyncScriptPerformingSleep()
    {
    ...
    //s click on element if visible - return boolean @@@@@@@
    public boolean doClickOnElement(WebElement element) {
        try {
            element.click();
            return true;
        }catch(Exception e)
    ...
    public void DoSendTextandPressEnter(By by,String text){
        Actions hover = new Actions(driver);
        WebElement destination = driver.findElement(by);
        destination.sendKeys(text + Keys.ENTER);
    }
    public void DoSendTextandPressEnter(WebElement element,String text){
        Actions hover = new Actions(driver);
    ...
    //========verify message
    public void DoVerifyElements(String displayMessage, String expectedMessage,WebElement element ) {
        String actualMessage = doGetTextFromElement(element);
        Assert.assertEquals(displayMessage, expectedMessage,actualMessage);
    }
    public void doVerifyElements(String expectedMessage, String by, String displayMessage) {
    ...
    //s boolean retur
    //  method for if element present or not
    public boolean doElementVisible(WebElement element) {
        try {
